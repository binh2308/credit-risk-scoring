...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{Time-series features} từ chuỗi thanh toán theo thời gian.
        \item \textit{Network features} từ mối quan hệ xã hội (social graph).
        \item \textit{Behavioral features} từ tương tác app, clickstream.
        \item \textit{Macroeconomic features} như lãi suất, unemployment rate.
        \item \textit{Psychometric features} từ khảo sát khách hàng.
    \end{itemize}
    ...
    \item \textbf{Hạn chế của SMOTE}:  Mặc dù SMOTE cải thiện, nhưng tỷ lệ 22\% default vẫn không đại diện hoàn hảo thực tế (một số tổ chức có tỷ lệ default 5-10\%).
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Tái huấn luyện model hằng tháng với dữ liệu mới và cập nhật siêu tham số.
    ...
    \item \textbf{Tối ưu hóa đa mục tiêu (Multi-objective Optimization):} Thay vì chỉ tối ưu hóa để tăng độ chính xác hay ROC, AUC có thể thiết kế hàm mục tiêu gắn liền với lợi nhuận kinh doanh (tối đa hóa số tiền thu về và giảm thiểu chi phí nợ xấu mất đi).
