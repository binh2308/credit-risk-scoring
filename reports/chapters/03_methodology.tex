...
Dựa trên các kết quả phân tích khám phá (EDA) đã thực hiện ở Chương 2, đặc biệt là tình trạng mất cân bằng phân lớp (Class Imbalance) trầm trọng và hiện tượng đa cộng tuyến (Multicollinearity) mạnh giữa các biến dư nợ hóa đơn, quy trình tiền xử lý và làm sạch dữ liệu (Data Cleaning) được thiết kế và thực hiện nghiêm ngặt theo các bước sau:
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{images/smote_distribution.png}
    \caption{Phân phối nhãn dữ liệu trên Tập Huấn luyện trước và sau khi áp dụng SMOTE}
    \label{fig:smote_dist}
\end{figure}
...
Trước khi tiến hành tuning với Optuna, dữ liệu huấn luyện tiếp tục được phân tách (hold-out) theo tỷ lệ Huấn luyện/Validation (80/20 của tập Huấn luyện gốc, thành 64\% Train và 16\% Validation tổng thể) nhằm mục đích kiểm định. Tập Validation 16\% này đóng vai trò giám khảo trong suốt quá trình Optuna thử nghiệm (với số vòng lặp \texttt{n\_trials=30}). Sau khi tìm được bộ tham số tối ưu, mô hình mới được huấn luyện lại lần cuối trên toàn bộ 80\% dữ liệu đại diện.

Optuna sử dụng thuật toán Tối ưu hóa Bayes, cụ thể là mô hình TPE (Tree-structured Parzen Estimator). Thuật toán này phân tích các vòng chạy (trials) trong quá khứ để xác định vùng không gian tham số tiềm năng nhất, từ đó hướng tới việc cực đại hóa điểm số $ROC\_AUC$. Các tham số cốt lõi và không gian tìm kiếm thiết lập cho mô hình bao gồm:

\begin{table}[H]
\centering
\caption{Không gian tìm kiếm Siêu tham số (Optuna Search Space)}
...
Siêu tham số & Biên độ tìm kiếm (Khoảng) & Ý nghĩa điều chỉnh \\
\texttt{max\_depth} & $[3, 8]$ & Độ sâu tối đa của cây, chống học vẹt (overfitting) \\
\texttt{learning\_rate} ($\eta$) & $[0.01, 0.3]$ & Tốc độ học mô hình (kích thước bước cập nhật) \\
\texttt{n\_estimators} & $[100, 400]$ & Số lượng cây quyết định tối đa được tạo ra \\
\texttt{subsample} & $[0.6, 1.0]$ & Tỷ lệ lấy mẫu ngẫu nhiên cho số hàng (dữ liệu dòng) \\
\texttt{colsample\_bytree} / \newline \texttt{feature\_fraction} & $[0.6, 1.0]$ & Tỷ lệ lấy mẫu đặc trưng cho từng cây \\
\end{tblr}
\end{table}

Chi tiết các tham số được đưa vào không gian tìm kiếm bao gồm:
\begin{itemize}
    ...
    \item \textbf{max\_depth \& n\_estimators:} Quản lý độ phức tạp cấu trúc cây, đóng vai trò là rào chắn quan trọng nhất để ngăn chặn tình trạng mô hình học vẹt (Overfitting) từ dữ liệu gốc.
    \item \textbf{subsample \& colsample\_bytree / feature\_fraction:} Tỷ lệ lấy mẫu con ngẫu nhiên đối với các hàng (data points) và cột (features). Cơ chế ngẫu nhiên hóa này giúp giảm phương sai (variance) của mô hình. Trong đó XGBoost sử dụng \texttt{colsample\_bytree}, LightGBM tối ưu bằng tính năng tương đương là \texttt{feature\_fraction}.
\end{itemize}
...
Công cụ SHAP cung cấp góc nhìn minh bạch và sâu sắc cho việc đánh giá mô hình bằng cách biểu diễn mức độ quan trọng bằng lượng (Feature Importance) và vạch rõ chiều tác động (Directional Impact) của mỗi biến lên xác suất vỡ nợ. Sự giải thích chuẩn xác này không chỉ giúp kiểm chứng logic toán học bên trong hệ thống mà còn đáp ứng trọn vẹn các tiêu chuẩn khắt khe về kiểm toán mô hình (Model Auditability), mang lại độ tin cậy để các chuyên gia tài chính có thể triển khai công nghệ vào ứng dụng xử lý tín dụng thực tế (các kết quả phân tích nội hàm chi tiết sử dụng kỹ thuật SHAP sẽ được minh họa cụ thể thông qua biểu đồ đánh giá trong Chương 4).
